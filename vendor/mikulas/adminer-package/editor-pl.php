...
upload_error($n){$md=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($n?'Wgranie pliku byÅ‚o niemoÅ¼liwe.'.($md?" ".sprintf('Maksymalna wielkoÅ›Ä‡ pliku to %sB.',$md):""):'Plik nie istnieje.');}function
...
lang($tf,$_d=null){if(is_array($tf)){$Zd=($_d==1?0:($_d%10>1&&$_d%10<5&&$_d/10%10!=1?1:2));$tf=$tf[$Zd];}$tf=str_replace("%d","%s",$tf);$_d=format_number($_d);return
...
h($g->error);}function
check_sqlite_name($D){global$g;$Tb="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Tb)\$~",$D)){$g->error=sprintf('ProszÄ™ uÅ¼yÄ‡ jednego z rozszerzeÅ„: %s.',str_replace("|",", ",$Tb));return
false;}return
true;}function
...
set_schema($Ae){global$g,$_f,$Ue;$K=$g->query("SET search_path TO ".idf_escape($Ae));foreach(types()as$U){if(!isset($_f[$U])){$_f[$U]=0;$Ue['Typy uÅ¼ytkownika'][]=$U;}}return$K;}function
...
unconvert_field($o,$K){return$K;}function
support($Wb){return
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Wb);}$y="oracle";$_f=array();$Ue=array();foreach(array('Numeryczne'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Data i czas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Tekstowe'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binarne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$z=>$X){$_f+=$X;$Ue[$z]=array_keys($X);}$Gf=array();$Id=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$nc=array("length","lower","round","upper");$pc=array("avg","count","count distinct","max","min","sum");$Bb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$xb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$ae=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
selectLinks($af,$P=""){$a=$af["Name"];if($P!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$P).'">'.'Nowy rekord'."</a>\n";}function
...
selectVal($X,$A,$o,$Nd){$K=($X===null?"&nbsp;":$X);$A=h($A);if(preg_match('~blob|bytea~',$o["type"])&&!is_utf8($X)){$K=lang(array('%d bajt','%d bajty','%d bajtÃ³w'),strlen($Nd));if(preg_match("~^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)~",$Nd))$K="<img src='$A' alt='$K'>";}if(like_bool($o)&&$K!="&nbsp;")$K=($X?'tak':'nie');if($A)$K="<a href='$A'".(is_url($A)?" rel='noreferrer'":"").">$K</a>";if(!$A&&!like_bool($o)&&preg_match('~int|float|double|decimal~',$o["type"]))$K="<div class='number'>$K</div>";elseif(preg_match('~date~',$o["type"]))$K="<div class='datetime'>$K</div>";return$K;}function
...
as$D=>$qb){$o=$p[$D];if(preg_match("~enum~",$o["type"])||like_bool($o)){$z=$Uc[$D];$t--;echo"<div>".h($qb)."<input type='hidden' name='where[$t][col]' value='".h($D)."'>:",(like_bool($o)?" <select name='where[$t][val]'>".optionlist(array(""=>"",'nie','tak'),$Z[$z]["val"],true)."</select>":enum_input("checkbox"," name='where[$t][val][]'",$o,(array)$Z[$z]["val"],($o["null"]?0:null))),"</div>\n";unset($f[$D]);}elseif(is_array($Kd=$this->_foreignKeyOptions($_GET["select"],$D))){if($p[$D]["null"])$Kd[0]='('.'puste'.')';$z=$Uc[$D];$t--;echo"<div>".h($qb)."<input type='hidden' name='where[$t][col]' value='".h($D)."'><input type='hidden' name='where[$t][op]' value='='>: <select name='where[$t][val]'>".optionlist($Kd,$Z[$z]["val"],true)."</select></div>\n";unset($f[$D]);}}$t=0;foreach($Z
...
as$z=>$w){$E=array();foreach($w["columns"]as$X)$E[]=$f[$X];if(count(array_filter($E,'strlen'))>1&&$z!="PRIMARY")$Md[$z]=implode(", ",$E);}if($Md){echo'<fieldset><legend>'.'Sortuj'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Md,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
as$Nc=>$X){if($X[0]<$jf)unset($Mc[$Nc]);}}$Lc=&$Mc[$b->bruteForceKey()];if(!$Lc)$Lc=array($jf+30*60,0);$Lc[1]++;$Ie=serialize($Mc);rewind($kc);fwrite($kc,$Ie);ftruncate($kc,strlen($Ie));flock($kc,LOCK_UN);fclose($kc);}$Ba=$_POST["auth"];if($Ba){$Mc=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$Lc=$Mc[$b->bruteForceKey()];$yd=($Lc[1]>30?$Lc[0]-time():0);if($yd>0)auth_error(lang(array('Za duÅ¼o nieudanych prÃ³b logowania, sprÃ³buj ponownie za %d minutÄ™.','Za duÅ¼o nieudanych prÃ³b logowania, sprÃ³buj ponownie za %d minuty.','Za duÅ¼o nieudanych prÃ³b logowania, sprÃ³buj ponownie za %d minut.'),ceil($yd/60)));session_regenerate_id();$m=$Ba["driver"];$O=$Ba["server"];$V=$Ba["username"];$H=(string)$Ba["password"];$l=$Ba["db"];set_password($m,$O,$V,$H);$_SESSION["db"][$m][$O][$V][$l]=true;if($Ba["permanent"]){$z=base64_encode($m)."-".base64_encode($O)."-".base64_encode($V)."-".base64_encode($l);$fe=$b->permanentLogin(true);$Wd[$z]="$z:".base64_encode($fe?encrypt_string($H,$fe):"");cookie("adminer_permanent",implode(" ",$Wd));}if(count($_POST)==1||DRIVER!=$m||SERVER!=$O||$_GET["username"]!==$V||DB!=$l)redirect(auth_url($m,$O,$V,$l));}elseif($_POST["logout"]){if($tc&&!verify_token()){page_header('Wyloguj','NieprawidÅ‚owy token CSRF. SprÃ³buj wysÅ‚aÄ‡ formularz ponownie.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$z)set_session($z,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'Wylogowano pomyÅ›lnie.');}}elseif($Wd&&!$_SESSION["pwds"]){session_regenerate_id();$fe=$b->permanentLogin();foreach($Wd
...
as$D=>$X){$X=process_input($p[$D]);if($X!==null&&($_POST["clone"]||$X!==false))$P[idf_escape($D)]=($X!==false?$X:idf_escape($D));}}if($_POST["delete"]||$P){if($_POST["clone"])$I="INTO ".table($a)." (".implode(", ",array_keys($P)).")\nSELECT ".implode(", ",$P)."\nFROM ".table($a);if($_POST["all"]||($Ff===array()&&is_array($_POST["check"]))||$Oc){$J=($_POST["delete"]?$m->delete($a,$Uf):($_POST["clone"]?queries("INSERT $I$Uf"):$m->update($a,$P,$Uf)));$ra=$g->affected_rows;}else{foreach((array)$_POST["check"]as$X){$Tf="\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p);$J=($_POST["delete"]?$m->delete($a,$Tf,1):($_POST["clone"]?queries("INSERT".limit1($I,$Tf)):$m->update($a,$P,$Tf)));if(!$J)break;$ra+=$g->affected_rows;}}}$pd=lang(array('Zmieniono %d rekord.','Zmieniono %d rekordy.','Zmieniono %d rekordÃ³w.'),$ra);if($_POST["clone"]&&$J&&$ra==1){$Zc=last_id();if($Zc)$pd=sprintf('Rekord%s zostaÅ‚ dodany.'," $Zc");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$pd,$J);if(!$_POST["delete"]){edit_form($a,$p,(array)$_POST["fields"],!$_POST["clone"]);page_footer();exit;}}elseif(!$_POST["import"]){if(!$_POST["val"])$n='Ctrl+kliknij wartoÅ›Ä‡, aby jÄ… edytowaÄ‡.';else{$J=true;$ra=0;foreach($_POST["val"]as$Df=>$L){$P=array();foreach($L
as$z=>$X){$z=bracket_escape($z,1);$P[idf_escape($z)]=(preg_match('~char|text~',$p[$z]["type"])||$X!=""?$b->processInput($p[$z],$X):"NULL");}$J=$m->update($a,$P," WHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($Df,$p),!($Oc||$Ff===array())," ");if(!$J)break;$ra+=$g->affected_rows;}queries_redirect(remove_from_uri(),lang(array('Zmieniono %d rekord.','Zmieniono %d rekordy.','Zmieniono %d rekordÃ³w.'),$ra),$J);}}elseif(!is_string($Xb=get_file("csv_file",true)))$n=upload_error($Xb);elseif(!preg_match('~~u',$Xb))$n='Kodowanie pliku musi byÄ‡ ustawione na UTF-8.';else{cookie("adminer_import","output=".urlencode($qa["output"])."&format=".urlencode($_POST["separator"]));$J=true;$Za=array_keys($p);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Xb,$jd);$ra=count($jd[0]);$m->begin();$He=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$M=array();foreach($jd[0]as$z=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$He]*)$He~",$X.$He,$kd);if(!$z&&!array_diff($kd[1],$Za)){$Za=$kd[1];$ra--;}else{$P=array();foreach($kd[1]as$t=>$Xa)$P[idf_escape($Za[$t])]=($Xa==""&&$p[$Za[$t]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Xa))));$M[]=$P;}}$J=(!$M||$m->insertUpdate($a,$M,$ce));if($J)$m->commit();queries_redirect(remove_from_uri("page"),lang(array('%d rekord zostaÅ‚ zaimportowany.','%d rekordy zostaÅ‚y zaimportowane.','%d rekordÃ³w zostaÅ‚o zaimportowanych.'),$ra),$J);$m->rollback();}}}$bf=$b->tableName($S);if(is_ajax()){page_headers();ob_start();}else
page_header('pokaÅ¼'.": $bf",$n);$P=null;if(isset($ye["insert"])||!support("table")){$P="";foreach((array)$_GET["where"]as$X){if(count($gc[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!preg_match('~[_%]~',$X["val"]))))$P.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($S,$P);if(!$f&&support("table"))echo"<p class='error'>".'Nie udaÅ‚o siÄ™ pobraÄ‡ danych z tabeli'.($p?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($N,$f);$b->selectSearchPrint($Z,$f,$x);$b->selectOrderPrint($E,$f,$x);$b->selectLimitPrint($_);$b->selectLengthPrint($hf);$b->selectActionPrint($x);echo"</form>\n";$F=$_GET["page"];if($F=="last"){$jc=$g->result(count_rows($a,$Z,$Oc,$s));$F=floor(max(0,$jc-1)/$_);}$Ee=$N;if(!$Ee){$Ee[]="*";if($Cd)$Ee[]=$Cd;}$gb=convert_fields($f,$p,$N);if($gb)$Ee[]=substr($gb,2);$J=$m->select($a,$Ee,$Z,$s,$E,$_,$F,true);if(!$J)echo"<p class='error'>".error()."\n";else{if($y=="mssql"&&$F)$J->seek($_*$F);$Gb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$M=array();while($L=$J->fetch_assoc()){if($F&&$y=="oracle")unset($L["RNUM"]);$M[]=$L;}if($_GET["page"]!="last"&&+$_&&$s&&$Oc&&$y=="sql")$jc=$g->result(" SELECT FOUND_ROWS()");if(!$M)echo"<p class='message'>".'Brak rekordÃ³w.'."\n";else{$Ha=$b->backwardKeys($a,$bf);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$s&&$N?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'ZmieÅ„'."</a>");$wd=array();$nc=array();reset($N);$ne=1;foreach($M[0]as$z=>$X){if($z!=$Cd){$X=$_GET["columns"][key($N)];$o=$p[$N?($X?$X["col"]:current($N)):$z];$D=($o?$b->fieldName($o,$ne):($X["fun"]?"*":$z));if($D!=""){$ne++;$wd[$z]=$D;$e=idf_escape($z);$zc=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($z);$qb="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($zc.($E[0]==$e||$E[0]==$z||(!$E&&$Oc&&$s[0]==$e)?$qb:'')).'">';echo
apply_sql_function($X["fun"],$D)."</a>";echo"<span class='column hidden'>","<a href='".h($zc.$qb)."' title='".'malejÄ…co'."' class='text'> â†“</a>";if(!$X["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($z)).'\'); return false;" title="'.'Szukaj'.'" class="text jsonly"> =</a>';echo"</span>";}$nc[$z]=$X["fun"];next($N);}}$cd=array();if($_GET["modify"]){foreach($M
as$L){foreach($L
as$z=>$X)$cd[$z]=max($cd[$z],min(40,strlen(utf8_decode($X))));}}echo($Ha?"<th>".'Relacje':"")."</thead>\n";if(is_ajax()){if($_%2==1&&$F%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($M,$gc)as$C=>$L){$Cf=unique_array($M[$C],$x);if(!$Cf){$Cf=array();foreach($M[$C]as$z=>$X){if(!preg_match('~^(COUNT\\((\\*|(DISTINCT )?`(?:[^`]|``)+`)\\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\\(`(?:[^`]|``)+`\\))$~',$z))$Cf[$z]=$X;}}$Df="";foreach($Cf
as$z=>$X){if(($y=="sql"||$y=="pgsql")&&strlen($X)>64){$z="MD5(".(strpos($z,'(')?$z:idf_escape($z)).")";$X=md5($X);}$Df.="&".($X!==null?urlencode("where[".bracket_escape($z)."]")."=".urlencode($X):"null%5B%5D=".urlencode($z));}echo"<tr".odd().">".(!$s&&$N?"":"<td>".checkbox("check[]",substr($Df,1),in_array(substr($Df,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($Oc||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Df)."'>".'edytuj'."</a>"));foreach($L
