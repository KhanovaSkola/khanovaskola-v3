...
enum_input($U,$Aa,$o,$Y,$Hb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$o["length"],$jd);$K=($Hb!==null?"<label><input type='$U'$Aa value='$Hb'".((is_array($Y)?in_array($Hb,$Y):$Y===0)?" checked":"")."><i>".'empty'."</i></label>":"");foreach($jd[1]as$t=>$X){$X=stripcslashes(str_replace("''","'",$X));$Qa=(is_int($Y)?$Y==$t+1:(is_array($Y)?in_array($t+1,$Y):$Y===$X));$K.=" <label><input type='$U'$Aa value='".($t+1)."'".($Qa?' checked':'').'>'.h($b->editVal($X,$o)).'</label>';}return$K;}function
...
h($g->error);}function
check_sqlite_name($D){global$g;$Tb="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Tb)\$~",$D)){$g->error=sprintf('Please use one of the extensions %s.',str_replace("|",", ",$Tb));return
false;}return
true;}function
...
set_schema($Ae){global$g,$_f,$Ue;$K=$g->query("SET search_path TO ".idf_escape($Ae));foreach(types()as$U){if(!isset($_f[$U])){$_f[$U]=0;$Ue['User types'][]=$U;}}return$K;}function
...
selectLinks($af,$P=""){$a=$af["Name"];if($P!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$P).'">'.'New item'."</a>\n";}function
...
as$z=>$w){$E=array();foreach($w["columns"]as$X)$E[]=$f[$X];if(count(array_filter($E,'strlen'))>1&&$z!="PRIMARY")$Md[$z]=implode(", ",$E);}if($Md){echo'<fieldset><legend>'.'Sort'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Md,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
as$Nc=>$X){if($X[0]<$jf)unset($Mc[$Nc]);}}$Lc=&$Mc[$b->bruteForceKey()];if(!$Lc)$Lc=array($jf+30*60,0);$Lc[1]++;$Ie=serialize($Mc);rewind($kc);fwrite($kc,$Ie);ftruncate($kc,strlen($Ie));flock($kc,LOCK_UN);fclose($kc);}$Ba=$_POST["auth"];if($Ba){$Mc=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$Lc=$Mc[$b->bruteForceKey()];$yd=($Lc[1]>30?$Lc[0]-time():0);if($yd>0)auth_error(lang(array('Too many unsuccessful logins, try again in %d minute.','Too many unsuccessful logins, try again in %d minutes.'),ceil($yd/60)));session_regenerate_id();$m=$Ba["driver"];$O=$Ba["server"];$V=$Ba["username"];$H=(string)$Ba["password"];$l=$Ba["db"];set_password($m,$O,$V,$H);$_SESSION["db"][$m][$O][$V][$l]=true;if($Ba["permanent"]){$z=base64_encode($m)."-".base64_encode($O)."-".base64_encode($V)."-".base64_encode($l);$fe=$b->permanentLogin(true);$Wd[$z]="$z:".base64_encode($fe?encrypt_string($H,$fe):"");cookie("adminer_permanent",implode(" ",$Wd));}if(count($_POST)==1||DRIVER!=$m||SERVER!=$O||$_GET["username"]!==$V||DB!=$l)redirect(auth_url($m,$O,$V,$l));}elseif($_POST["logout"]){if($tc&&!verify_token()){page_header('Logout','Invalid CSRF token. Send the form again.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$z)set_session($z,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'Logout successful.');}}elseif($Wd&&!$_SESSION["pwds"]){session_regenerate_id();$fe=$b->permanentLogin();foreach($Wd
...
as$D=>$o){if(!isset($o["privileges"][$Hf?"update":"insert"])||$b->fieldName($o)=="")unset($p[$D]);}if($_POST&&!$n&&!isset($_GET["select"])){$ed=$_POST["referer"];if($_POST["insert"])$ed=($Hf?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$ed))$ed=ME."select=".urlencode($a);$x=indexes($a);$Cf=unique_array($_GET["where"],$x);$le="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($ed,'Item has been deleted.',$m->delete($a,$le,!$Cf));else{$P=array();foreach($p
as$D=>$o){$X=process_input($o);if($X!==false&&$X!==null)$P[idf_escape($D)]=$X;}if($Hf){if(!$P)redirect($ed);queries_redirect($ed,'Item has been updated.',$m->update($a,$P,$le,!$Cf));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$J=$m->insert($a,$P);$Zc=($J?last_id():0);queries_redirect($ed,sprintf('Item%s has been inserted.',($Zc?" $Zc":"")),$J);}}}$L=null;if($_POST["save"])$L=(array)$_POST["fields"];elseif($Z){$N=array();foreach($p
