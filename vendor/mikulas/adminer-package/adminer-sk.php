...
',(support("comment")?"<td".($rb?"":" class='hidden'").">".'KomentÃ¡r':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.2.0' alt='+' title='".'PridaÅ¥ ÄalÅ¡Ã­'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
...
',(support("comment")?"<td".($rb?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($n["comment"])."' maxlength='".($g->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.2.0' alt='+' title='".'PridaÅ¥ ÄalÅ¡Ã­'."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=4.2.0' alt='^' title='".'PresunÃºÅ¥ hore'."'>&nbsp;"."<input type='image' class='icon' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=4.2.0' alt='v' title='".'PresunÃºÅ¥ dolu'."'>&nbsp;":""),($Ye==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.2.0' alt='x' title='".'OdobraÅ¥'."' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">":""),"\n";}}function
...
<input type="submit" value="UloÅ¾iÅ¥">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'OdstrÃ¡niÅ¥'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.2.0' alt='+' title='".'PridaÅ¥ ÄalÅ¡Ã­'."'>\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
as$X){if($X[1]!="USAGE")$Vc["$B[2]$X[2]"][$X[1]]=true;if(preg_match('~ WITH GRANT OPTION~',$K[0]))$Vc["$B[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$B))$He=$B[1];}}if($_POST&&!$m){$Ie=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");if($_POST["drop"])query_redirect("DROP USER $Ie",ME."privileges=",'PouÅ¾Ã­vateÄ¾ bol odstrÃ¡nenÃ½.');else{$ye=q($_POST["user"])."@".q($_POST["host"]);$nf=$_POST["pass"];if($nf!=''&&!$_POST["hashed"]){$nf=$g->result("SELECT PASSWORD(".q($nf).")");$m=!$nf;}$Cb=false;if(!$m){if($Ie!=$ye){$Cb=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $ye IDENTIFIED BY PASSWORD ".q($nf));$m=!$Cb;}elseif($nf!=$He)queries("SET PASSWORD FOR $ye = ".q($nf));}if(!$m){$cg=array();foreach($we
