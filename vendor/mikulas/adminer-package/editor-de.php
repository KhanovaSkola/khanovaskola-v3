...
upload_error($m){$pd=($m==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($m?'Hochladen von Datei fehlgeschlagen.'.($pd?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$pd):""):'Datei existiert nicht.');}function
...
set_schema($Ce){global$g,$Bf,$We;$K=$g->query("SET search_path TO ".idf_escape($Ce));foreach(types()as$U){if(!isset($Bf[$U])){$Bf[$U]=0;$We['Benutzer-definierte Typen'][]=$U;}}return$K;}function
