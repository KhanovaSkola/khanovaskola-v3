...
upload_error($m){$de=($m==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($m?'NepodaÅ™ilo se nahrÃ¡t soubor.'.($de?" ".sprintf('MaximÃ¡lnÃ­ povolenÃ¡ velikost souboru je %sB.',$de):""):'Soubor neexistuje.');}function
...
lang($nh,$ze=null){if(is_array($nh)){$qf=($ze==1?0:($ze&&$ze<5?1:2));$nh=$nh[$qf];}$nh=str_replace("%d","%s",$nh);$ze=format_number($ze);return
...
h($g->error);}function
check_sqlite_name($C){global$g;$yc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($yc)\$~",$C)){$g->error=sprintf('ProsÃ­m pouÅ¾ijte jednu z koncovek %s.',str_replace("|",", ",$yc));return
false;}return
true;}function
...
set_schema($hg){global$g,$vh,$Eg;$J=$g->query("SET search_path TO ".idf_escape($hg));foreach(types()as$U){if(!isset($vh[$U])){$vh[$U]=0;$Eg['UÅ¾ivatelskÃ© typy'][]=$U;}}return$J;}function
...
unconvert_field($n,$J){return$J;}function
support($Ac){return
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Ac);}$w="oracle";$vh=array();$Eg=array();foreach(array('ÄŒÃ­sla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum a Äas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Å˜etÄ›zce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'BinÃ¡rnÃ­'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$x=>$X){$vh+=$X;$Eg[$x]=array_keys($X);}$Bh=array();$Le=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$Qc=array("length","lower","round","upper");$Vc=array("avg","count","count distinct","max","min","sum");$ac=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Sb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$tf=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
selectLinks($Lg,$O=""){echo'<p class="links">';$Td=array("select"=>'Vypsat data');if(support("table")||support("indexes"))$Td["table"]='Zobrazit strukturu';if(support("table")){if(is_view($Lg))$Td["view"]='PozmÄ›nit pohled';else$Td["create"]='PozmÄ›nit tabulku';}if($O!==null)$Td["edit"]='NovÃ¡ poloÅ¾ka';foreach($Td
...
selectQuery($H,$bh){global$w;return"<p><code class='jush-$w'>".h(str_replace("\n"," ",$H))."</code> <span class='time'>($bh)</span>".(support("sql")?" <a href='".h(ME)."sql=".urlencode($H)."'>".'Upravit'."</a>":"")."</p>";}function
rowDescription($Q){return"";}function
rowDescriptions($L,$Ic){return$L;}function
selectLink($X,$n){}function
selectVal($X,$_,$n,$Xe){$J=($X===null?"<i>NULL</i>":(preg_match("~char|binary~",$n["type"])&&!preg_match("~var~",$n["type"])?"<code>$X</code>":$X));if(preg_match('~blob|bytea|raw|file~',$n["type"])&&!is_utf8($X))$J=lang(array('%d bajt','%d bajty','%d bajtÅ¯'),strlen($Xe));return($_?"<a href='".h($_)."'".(is_url($_)?" rel='noreferrer'":"").">$J</a>":$J);}function
editVal($X,$n){return$X;}function
selectColumnsPrint($M,$f){global$Qc,$Vc;print_fieldset("select",'Vypsat',$M);$r=0;$M[""]=array();foreach($M
as$x=>$X){$X=$_GET["columns"][$x];$e=select_input(" name='columns[$r][col]' onchange='".($x!==""?"selectFieldChange(this.form)":"selectAddRow(this)").";'",$f,$X["col"]);echo"<div>".($Qc||$Vc?"<select name='columns[$r][fun]' onchange='helpClose();".($x!==""?"":" this.nextSibling.nextSibling.onchange();")."'".on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'",1).">".optionlist(array(-1=>"")+array_filter(array('Funkce'=>$Qc,'Agregace'=>$Vc)),$X["fun"])."</select>"."($e)":$e)."</div>\n";$r++;}echo"</div></fieldset>\n";}function
...
selectOrderPrint($Qe,$f,$v){print_fieldset("sort",'SeÅ™adit',$Qe);$r=0;foreach((array)$_GET["order"]as$x=>$X){if($X!=""){echo"<div>".select_input(" name='order[$r]' onchange='selectFieldChange(this.form);'",$f,$X),checkbox("desc[$r]",1,isset($_GET["desc"][$x]),'sestupnÄ›')."</div>\n";$r++;}}echo"<div>".select_input(" name='order[$r]' onchange='selectAddRow(this);'",$f),checkbox("desc[$r]",1,false,'sestupnÄ›')."</div>\n","</div></fieldset>\n";}function
...
selectLengthPrint($ah){if($ah!==null){echo"<fieldset><legend>".'DÃ©lka textÅ¯'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($ah)."'>","</div></fieldset>\n";}}function
...
messageQuery($H,$bh){global$w;restart_session();$ad=&get_session("queries");$s="sql-".count($ad[$_GET["db"]]);if(strlen($H)>1e6)$H=preg_replace('~[\x80-\xFF]+$~','',substr($H,0,1e6))."\n...";$ad[$_GET["db"]][]=array($H,time(),$bh);return" <span class='time'>".@date("H:i:s")."</span> <a href='#$s' onclick=\"return !toggle('$s');\">".'SQL pÅ™Ã­kaz'."</a>"."<div id='$s' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($H,1000).'</code></pre>'.($bh?" <span class='time'>($bh)</span>":'').(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($ad[$_GET["db"]])-1)).'">'.'Upravit'.'</a>':'').'</div>';}function
...
';}$this->databasesPrint($oe);if(DB==""||!$oe){echo"<p class='links'>".(support("sql")?"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])&&!isset($_GET["import"])).">".'SQL pÅ™Ã­kaz'."</a>\n<a href='".h(ME)."import='".bold(isset($_GET["import"])).">".'Import'."</a>\n":"")."";if(support("dump"))echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Export'."</a>\n";}if($_GET["ns"]!==""&&!$oe&&DB!=""){echo'<a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'VytvoÅ™it tabulku'."</a>\n";if(!$S)echo"<p class='message'>".'Å½Ã¡dnÃ© tabulky.'."\n";else$this->tablesPrint($S);}}}function
databasesPrint($oe){global$b,$g;$j=$this->databases();echo'<form action="">
<p id="dbs">
';hidden_fields_get();$Eb=" onmousedown='dbMouseDown(event, this);' onchange='dbChange(this);'";echo"<span title='".'databÃ¡ze'."'>DB</span>: ".($j?"<select name='db'$Eb>".optionlist(array(""=>"")+$j,DB)."</select>":'<input name="db" value="'.h(DB).'" autocapitalize="off">'),"<input type='submit' value='".'Vybrat'."'".($j?" class='hidden'":"").">\n";if($oe!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".'SchÃ©ma'.": <select name='ns'$Eb>".optionlist(array(""=>"")+$b->schemas(),$_GET["ns"])."</select>";if($_GET["ns"]!="")set_schema($_GET["ns"]);}}echo(isset($_GET["sql"])?'<input type="hidden" name="sql" value="">':(isset($_GET["schema"])?'<input type="hidden" name="schema" value="">':(isset($_GET["dump"])?'<input type="hidden" name="dump" value="">':(isset($_GET["privileges"])?'<input type="hidden" name="privileges" value="">':"")))),"</p></form>\n";}function
tablesPrint($S){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($S
as$Q=>$Bg){echo'<a href="'.h(ME).'select='.urlencode($Q).'"'.bold($_GET["select"]==$Q||$_GET["edit"]==$Q).">".'vypsat'."</a> ";$C=$this->tableName($Bg);echo(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($Q).'"'.bold(in_array($Q,array($_GET["table"],$_GET["create"],$_GET["indexes"],$_GET["foreign"],$_GET["trigger"])),(is_view($Bg)?"view":""))." title='".'Zobrazit strukturu'."'>$C</a>":"<span>$C</span>")."<br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
...
as$vd=>$X){if($X[0]<$bh)unset($ud[$vd]);}}$td=&$ud[$b->bruteForceKey()];if(!$td)$td=array($bh+30*60,0);$td[1]++;$qg=serialize($ud);rewind($Nc);fwrite($Nc,$qg);ftruncate($Nc,strlen($qg));flock($Nc,LOCK_UN);fclose($Nc);}$Ia=$_POST["auth"];if($Ia){$ud=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$td=$ud[$b->bruteForceKey()];$xe=($td[1]>30?$td[0]-time():0);if($xe>0)auth_error(lang(array('PÅ™Ã­liÅ¡ mnoho pokusÅ¯ o pÅ™ihlÃ¡Å¡enÃ­, zkuste to znovu za %d minutu.','PÅ™Ã­liÅ¡ mnoho pokusÅ¯ o pÅ™ihlÃ¡Å¡enÃ­, zkuste to znovu za %d minuty.','PÅ™Ã­liÅ¡ mnoho pokusÅ¯ o pÅ™ihlÃ¡Å¡enÃ­, zkuste to znovu za %d minut.'),ceil($xe/60)));session_regenerate_id();$l=$Ia["driver"];$N=$Ia["server"];$V=$Ia["username"];$G=(string)$Ia["password"];$k=$Ia["db"];set_password($l,$N,$V,$G);$_SESSION["db"][$l][$N][$V][$k]=true;if($Ia["permanent"]){$x=base64_encode($l)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($k);$zf=$b->permanentLogin(true);$nf[$x]="$x:".base64_encode($zf?encrypt_string($G,$zf):"");cookie("adminer_permanent",implode(" ",$nf));}if(count($_POST)==1||DRIVER!=$l||SERVER!=$N||$_GET["username"]!==$V||DB!=$k)redirect(auth_url($l,$N,$V,$k));}elseif($_POST["logout"]){if($Zc&&!verify_token()){page_header('OdhlÃ¡sit','NeplatnÃ½ token CSRF. OdeÅ¡lete formulÃ¡Å™ znovu.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$x)set_session($x,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'OdhlÃ¡Å¡enÃ­ probÄ›hlo v poÅ™Ã¡dku.');}}elseif($nf&&!$_SESSION["pwds"]){session_regenerate_id();$zf=$b->permanentLogin();foreach($nf
...
auth_error($m){global$b,$Zc;$tg=session_name();if(!$_COOKIE[$tg]&&$_GET[$tg]&&ini_bool("session.use_only_cookies"))$m='Session promÄ›nnÃ© musÃ­ bÃ½t povolenÃ©.';elseif(isset($_GET["username"])){if(($_COOKIE[$tg]||$_GET[$tg])&&!$Zc)$m='Session vyprÅ¡ela, pÅ™ihlaÅ¡te se prosÃ­m znovu.';else{add_invalid_login();$G=get_password();if($G!==null){if($G===false)$m.='<br>'.sprintf('Platnost hlavnÃ­ho hesla vyprÅ¡ela. <a href="http://www.adminer.org/cs/extension/" target="_blank">Implementujte</a> metodu %s, aby platilo stÃ¡le.','<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}$F=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$F["lifetime"]);page_header('PÅ™ihlÃ¡sit se',$m,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Å½Ã¡dnÃ© rozÅ¡Ã­Å™enÃ­',sprintf('NenÃ­ dostupnÃ© Å¾Ã¡dnÃ© z podporovanÃ½ch PHP rozÅ¡Ã­Å™enÃ­ (%s).',implode(", ",$tf)),false);page_footer("auth");exit;}$g=connect();}$l=new
Min_Driver($g);if(!is_object($g)||!$b->login($_GET["username"],get_password()))auth_error((is_string($g)?$g:'NeplatnÃ© pÅ™ihlaÅ¡ovacÃ­ Ãºdaje.'));if($Ia&&$_POST["token"])$_POST["token"]=$T;$m='';if($_POST){if(!verify_token()){$od="max_input_vars";$ee=ini_get($od);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$x){$X=ini_get($x);if($X&&(!$ee||$X<$ee)){$od=$x;$ee=$X;}}}$m=(!$_POST["token"]&&$ee?sprintf('Byl pÅ™ekroÄen maximÃ¡lnÃ­ povolenÃ½ poÄet polÃ­. ZvyÅ¡te prosÃ­m %s.',"'$od'"):'NeplatnÃ½ token CSRF. OdeÅ¡lete formulÃ¡Å™ znovu.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$m=sprintf('PÅ™Ã­liÅ¡ velkÃ¡ POST data. ZmenÅ¡ete data nebo zvyÅ¡te hodnotu konfiguraÄnÃ­ direktivy %s.',"'post_max_size'");if(isset($_GET["sql"]))$m.=' '.'VelkÃ½ SQL soubor mÅ¯Å¾ete nahrÃ¡t pomocÃ­ FTP a importovat ho ze serveru.';}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
...
as$x=>$X){if($X===null)$X="<i>NULL</i>";elseif($Ra[$x]&&!is_utf8($X))$X="<i>".lang(array('%d bajt','%d bajty','%d bajtÅ¯'),strlen($X))."</i>";elseif(!strlen($X))$X="&nbsp;";else{$X=h($X);if($vh[$x]==254)$X="<code>$X</code>";}if(isset($Td[$x])&&!$f[$Td[$x]]){if($Te&&$w=="sql"){$Q=$K[array_search("table=",$Td)];$_=$Td[$x].urlencode($Te[$Q]!=""?$Te[$Q]:$Q);}else{$_="edit=".urlencode($Td[$x]);foreach($v[$Td[$x]]as$hb=>$Ad)$_.="&where".urlencode("[".bracket_escape($hb)."]")."=".urlencode($K[$Ad]);}$X="<a href='".h(ME.$_)."'>$X</a>";}echo"<td>$X";}}echo($r?"</table>":"<p class='message'>".'Å½Ã¡dnÃ© Å™Ã¡dky.')."\n";return$J;}function
...
edit_type($x,$n,$jb,$Jc=array()){global$Eg,$vh,$Bh,$He;$U=$n["type"];echo'<td><select name="',$x,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);"',on_help("getTarget(event).value",1),'>';if($U&&!isset($vh[$U])&&!isset($Jc[$U]))array_unshift($Eg,$U);if($Jc)$Eg['CizÃ­ klÃ­Äe']=$Jc;echo
optionlist($Eg,$U),'</select>
<td><input name="',$x,'[length]" value="',h($n["length"]),'" size="3" onfocus="editingLengthFocus(this);"',(!$n["length"]&&preg_match('~var(char|binary)$~',$U)?" class='required'":""),' onchange="editingLengthChange(this);" onkeyup="this.onchange();"><td class="options">';echo"<select name='$x"."[collation]'".(preg_match('~(char|text|enum|set)$~',$U)?"":" class='hidden'").'><option value="">('.'porovnÃ¡vÃ¡nÃ­'.')'.optionlist($jb,$n["collation"]).'</select>',($Bh?"<select name='$x"."[unsigned]'".(!$U||preg_match('~((^|[^o])int|float|double|decimal)$~',$U)?"":" class='hidden'").'><option>'.optionlist($Bh,$n["unsigned"]).'</select>':''),(isset($n['on_update'])?"<select name='$x"."[on_update]'".(preg_match('~timestamp|datetime~',$U)?"":" class='hidden'").'>'.optionlist(array(""=>"(".'PÅ™i zmÄ›nÄ›'.")","CURRENT_TIMESTAMP"),$n["on_update"]).'</select>':''),($Jc?"<select name='$x"."[on_delete]'".(preg_match("~`~",$U)?"":" class='hidden'")."><option value=''>(".'PÅ™i smazÃ¡nÃ­'.")".optionlist(explode("|",$He),$n["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($ob?"":" class='hidden'").">".'KomentÃ¡Å™':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.1.0' alt='+' title='".'PÅ™idat dalÅ¡Ã­'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
...
',(support("comment")?"<td".($ob?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($n["comment"])."' maxlength='".($g->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.1.0' alt='+' title='".'PÅ™idat dalÅ¡Ã­'."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=4.1.0' alt='^' title='".'PÅ™esunout nahoru'."'>&nbsp;"."<input type='image' class='icon' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=4.1.0' alt='v' title='".'PÅ™esunout dolÅ¯'."'>&nbsp;":""),($Ue==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.1.0' alt='x' title='".'Odebrat'."' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">":""),"\n";}}function
...
as$x=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($x)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($x))."'>".'ZmÄ›nit'."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.'PÅ™idat trigger'."</a>\n";}}elseif(isset($_GET["schema"])){page_header('SchÃ©ma databÃ¡ze',"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$Og=array();$Pg=array();$C="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$C-".DB]?"$C-".DB:$C)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Yd,PREG_SET_ORDER);foreach($Yd
...
as$C=>$n){if(!isset($n["privileges"][$Ch?"update":"insert"])||$b->fieldName($n)=="")unset($o[$C]);}if($_POST&&!$m&&!isset($_GET["select"])){$A=$_POST["referer"];if($_POST["insert"])$A=($Ch?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$A))$A=ME."select=".urlencode($a);$v=indexes($a);$yh=unique_array($_GET["where"],$v);$Hf="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($A,'PoloÅ¾ka byla smazÃ¡na.',$l->delete($a,$Hf,!$yh));else{$O=array();foreach($o
as$C=>$n){$X=process_input($n);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($Ch){if(!$O)redirect($A);queries_redirect($A,'PoloÅ¾ka byla aktualizovÃ¡na.',$l->update($a,$O,$Hf,!$yh));if(is_ajax()){page_headers();page_messages($m);exit;}}else{$I=$l->insert($a,$O);$Ld=($I?last_id():0);queries_redirect($A,sprintf('PoloÅ¾ka%s byla vloÅ¾ena.',($Ld?" $Ld":"")),$I);}}}$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($o
...
',checkbox("defaults",1,true,'VÃ½chozÃ­ hodnoty',"columnShow(this.checked, 5)","jsonly");if(!$_POST["defaults"]){echo'<script type="text/javascript">editingHideDefaults()</script>';}echo(support("comment")?"<label><input type='checkbox' name='comments' value='1' class='jsonly' onclick=\"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();\"".($ob?" checked":"").">".'KomentÃ¡Å™'."</label>".' <input name="Comment" id="Comment" value="'.h($K["Comment"]).'" maxlength="'.($g->server_info>=5.5?2048:60).'"'.($ob?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="UloÅ¾it">
';}echo'
';if($a!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}if(support("partitioning")){$gf=preg_match('~RANGE|LIST~',$K["partition_by"]);print_fieldset("partition",'RozdÄ›lit podle',$K["partition_by"]);echo'<p>
...
OddÃ­ly: <input type="number" name="partitions" class="size',($gf||!$K["partition_by"]?" hidden":""),'" value="',h($K["partitions"]),'">
...
as$k){if(count($j)==1||$k!=""){if(!create_database($k,$K["collation"]))$Gg=false;$Kd=$k;}}queries_redirect(ME."db=".urlencode($Kd),'DatabÃ¡ze byla vytvoÅ™ena.',$Gg);}}else{if(!$K["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i',$K["collation"])?" COLLATE $K[collation]":""),substr(ME,0,-1),'DatabÃ¡ze byla zmÄ›nÄ›na.');}}page_header(DB!=""?'PozmÄ›nit databÃ¡zi':'VytvoÅ™it databÃ¡zi',$m,array(),h(DB));$jb=collations();$C=DB;if($_POST)$C=$K["name"];elseif(DB!="")$K["collation"]=db_collation(DB,$jb);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$Rc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$Rc,$B)&&$B[1]){$C=stripcslashes(idf_unescape("`$B[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($C,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($C).'</textarea><br>':'<input name="name" id="name" value="'.h($C).'" maxlength="64" autocapitalize="off">')."\n".($jb?html_select("collation",array(""=>"(".'porovnÃ¡vÃ¡nÃ­'.")")+$jb,$K["collation"]).doc_link(array('sql'=>"charset-charsets.html",'mssql'=>"ms187963.aspx",)):"");?>
<script type='text/javascript'>focus(document.getElementById('name'));</script>
<input type="submit" value="UloÅ¾it">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'Odstranit'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.1.0' alt='+' title='".'PÅ™idat dalÅ¡Ã­'."'>\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
<p>
PÅ™i smazÃ¡nÃ­: ',html_select("on_delete",array(-1=>"")+explode("|",$He),$K["on_delete"]),' PÅ™i zmÄ›nÄ›: ',html_select("on_update",array(-1=>"")+explode("|",$He),$K["on_update"]),doc_link(array('sql'=>"innodb-foreign-key-constraints.html",'pgsql'=>"sql-createtable.html#SQL-CREATETABLE-REFERENCES",'mssql'=>"ms174979.aspx",'oracle'=>"clauses002.htm#sthref2903",)),'<p>
<input type="submit" value="UloÅ¾it">
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$K=$_POST;if($_POST&&!$m){$C=trim($K["name"]);$Ea=" AS\n$K[select]";$A=ME."table=".urlencode($C);$ge='Pohled byl zmÄ›nÄ›n.';if(!$_POST["drop"]&&$a==$C&&$w!="sqlite")query_redirect(($w=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ea,$A,$ge);else{$Wg=$C."_adminer_".uniqid();drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($C).$Ea,"DROP VIEW ".table($C),"CREATE VIEW ".table($Wg).$Ea,"DROP VIEW ".table($Wg),($_POST["drop"]?substr(ME,0,-1):$A),'Pohled byl odstranÄ›n.',$ge,'Pohled byl vytvoÅ™en.',$a,$C);}}if(!$_POST&&$a!=""){$K=view($a);$K["name"]=$a;if(!$m)$m=$g->error;}page_header(($a!=""?'PozmÄ›nit pohled':'VytvoÅ™it pohled'),$m,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
<tr><th>KaÅ¾dÃ½ch<td><input type="number" name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$sd,$K["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$Cg,$K["STATUS"]),'<tr><th>KomentÃ¡Å™<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
...
';}elseif(isset($_GET["procedure"])){$da=$_GET["procedure"];$bg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$K=$_POST;$K["fields"]=(array)$K["fields"];if($_POST&&!process_fields($K["fields"])&&!$m){$Wg="$K[name]_adminer_".uniqid();drop_create("DROP $bg ".idf_escape($da),create_routine($bg,$K),"DROP $bg ".idf_escape($K["name"]),create_routine($bg,array("name"=>$Wg)+$K),"DROP $bg ".idf_escape($Wg),substr(ME,0,-1),'Procedura byla odstranÄ›na.','Procedura byla zmÄ›nÄ›na.','Procedura byla vytvoÅ™ena.',$da,$K["name"]);}page_header(($da!=""?(isset($_GET["function"])?'ZmÄ›nit funkci':'ZmÄ›nit proceduru').": ".h($da):(isset($_GET["function"])?'VytvoÅ™it funkci':'VytvoÅ™it proceduru')),$m);if(!$_POST&&$da!=""){$K=routine($da,$bg);$K["name"]=$da;}$jb=get_vals("SHOW CHARACTER SET");sort($jb);$cg=routine_languages();echo'
<form action="" method="post" id="form">
<p>NÃ¡zev: <input name="name" value="',h($K["name"]),'" maxlength="64" autocapitalize="off">
',($cg?'Jazyk'.": ".html_select("language",$cg,$K["language"]):""),'<input type="submit" value="UloÅ¾it">
<table cellspacing="0" class="nowrap">
';edit_fields($K["fields"],$jb,$bg);if(isset($_GET["function"])){echo"<tr><td>".'NÃ¡vratovÃ½ typ';edit_type("returns",$K["returns"],$jb);}echo'</table>
...
as$X){if($X[1]!="USAGE")$Sc["$B[2]$X[2]"][$X[1]]=true;if(preg_match('~ WITH GRANT OPTION~',$K[0]))$Sc["$B[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$B))$Ee=$B[1];}}if($_POST&&!$m){$Fe=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");if($_POST["drop"])query_redirect("DROP USER $Fe",ME."privileges=",'UÅ¾ivatel byl odstranÄ›n.');else{$ve=q($_POST["user"])."@".q($_POST["host"]);$jf=$_POST["pass"];if($jf!=''&&!$_POST["hashed"]){$jf=$g->result("SELECT PASSWORD(".q($jf).")");$m=!$jf;}$_b=false;if(!$m){if($Fe!=$ve){$_b=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $ve IDENTIFIED BY PASSWORD ".q($jf));$m=!$_b;}elseif($jf!=$Ee)queries("SET PASSWORD FOR $ve = ".q($jf));}if(!$m){$Yf=array();foreach($te
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'>".'OprÃ¡vnÄ›nÃ­'.doc_link(array('sql'=>"grant.html#priv_level"));$r=0;foreach($Sc
as$_e=>$Rc){echo'<th>'.($_e!="*.*"?"<input name='objects[$r]' value='".h($_e)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'DatabÃ¡ze',"Tables"=>'Tabulka',"Columns"=>'Sloupec',"Procedures"=>'Procedura',)as$ub=>$Lb){foreach((array)$Af[$ub]as$_f=>$nb){echo"<tr".odd()."><td".($Lb?">$Lb<td":" colspan='2'").' lang="en" title="'.h($nb).'">'.h($_f);$r=0;foreach($Sc
as$_e=>$Rc){$C="'grants[$r][".h(strtoupper($_f))."]'";$Y=$Rc[strtoupper($_f)];if($ub=="Server Admin"&&$_e!=(isset($Sc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".'Povolit'."<option value='0'".($Y=="0"?" selected":"").">".'ZakÃ¡zat'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$m){$Hd=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X)))$Hd++;}queries_redirect(ME."processlist=",lang(array('Byl ukonÄen %d proces.','Byly ukonÄeny %d procesy.','Bylo ukonÄeno %d procesÅ¯.'),$Hd),$Hd||!$_POST["kill"]);}page_header('Seznam procesÅ¯',$m);echo'
<form action="" method="post">
...
as$C=>$X){$X=process_input($o[$C]);if($X!==null&&($_POST["clone"]||$X!==false))$O[idf_escape($C)]=($X!==false?$X:idf_escape($C));}}if($_POST["delete"]||$O){if($_POST["clone"])$H="INTO ".table($a)." (".implode(", ",array_keys($O)).")\nSELECT ".implode(", ",$O)."\nFROM ".table($a);if($_POST["all"]||($Ah===array()&&is_array($_POST["check"]))||$wd){$I=($_POST["delete"]?$l->delete($a,$Uh):($_POST["clone"]?queries("INSERT $H$Uh"):$l->update($a,$O,$Uh)));$xa=$g->affected_rows;}else{foreach((array)$_POST["check"]as$X){$Th="\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$o);$I=($_POST["delete"]?$l->delete($a,$Th,1):($_POST["clone"]?queries("INSERT".limit1($H,$Th)):$l->update($a,$O,$Th)));if(!$I)break;$xa+=$g->affected_rows;}}}$ge=lang(array('Byl ovlivnÄ›n %d zÃ¡znam.','Byly ovlivnÄ›ny %d zÃ¡znamy.','Bylo ovlivnÄ›no %d zÃ¡znamÅ¯.'),$xa);if($_POST["clone"]&&$I&&$xa==1){$Ld=last_id();if($Ld)$ge=sprintf('PoloÅ¾ka%s byla vloÅ¾ena.'," $Ld");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$ge,$I);if(!$_POST["delete"]){edit_form($a,$o,(array)$_POST["fields"],!$_POST["clone"]);page_footer();exit;}}elseif(!$_POST["import"]){if(!$_POST["val"])$m='Ctrl+kliknÄ›te na polÃ­Äko, kterÃ© chcete zmÄ›nit.';else{$I=true;$xa=0;foreach($_POST["val"]as$zh=>$K){$O=array();foreach($K
as$x=>$X){$x=bracket_escape($x,1);$O[idf_escape($x)]=(preg_match('~char|text~',$o[$x]["type"])||$X!=""?$b->processInput($o[$x],$X):"NULL");}$I=$l->update($a,$O," WHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($zh,$o),!($wd||$Ah===array())," ");if(!$I)break;$xa+=$g->affected_rows;}queries_redirect(remove_from_uri(),lang(array('Byl ovlivnÄ›n %d zÃ¡znam.','Byly ovlivnÄ›ny %d zÃ¡znamy.','Bylo ovlivnÄ›no %d zÃ¡znamÅ¯.'),$xa),$I);}}elseif(!is_string($Cc=get_file("csv_file",true)))$m=upload_error($Cc);elseif(!preg_match('~~u',$Cc))$m='Soubor musÃ­ bÃ½t v kÃ³dovÃ¡nÃ­ UTF-8.';else{cookie("adminer_import","output=".urlencode($wa["output"])."&format=".urlencode($_POST["separator"]));$I=true;$kb=array_keys($o);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Cc,$Yd);$xa=count($Yd[0]);$l->begin();$og=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$L=array();foreach($Yd[0]as$x=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$og]*)$og~",$X.$og,$Zd);if(!$x&&!array_diff($Zd[1],$kb)){$kb=$Zd[1];$xa--;}else{$O=array();foreach($Zd[1]as$r=>$hb)$O[idf_escape($kb[$r])]=($hb==""&&$o[$kb[$r]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$hb))));$L[]=$O;}}$I=(!$L||$l->insertUpdate($a,$L,$wf));if($I)$l->commit();queries_redirect(remove_from_uri("page"),lang(array('Byl importovÃ¡n %d zÃ¡znam.','Byly importovÃ¡ny %d zÃ¡znamy.','Bylo importovÃ¡no %d zÃ¡znamÅ¯.'),$xa),$I);$l->rollback();}}}$Mg=$b->tableName($R);if(is_ajax()){page_headers();ob_start();}else
page_header('Vypsat'.": $Mg",$m);$O=null;if(isset($Zf["insert"])||!support("table")){$O="";foreach((array)$_GET["where"]as$X){if(count($Jc[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!preg_match('~[_%]~',$X["val"]))))$O.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($R,$O);if(!$f&&support("table"))echo"<p class='error'>".'NepodaÅ™ilo se vypsat tabulku'.($o?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($M,$f);$b->selectSearchPrint($Z,$f,$v);$b->selectOrderPrint($Qe,$f,$v);$b->selectLimitPrint($z);$b->selectLengthPrint($ah);$b->selectActionPrint($v);echo"</form>\n";$E=$_GET["page"];if($E=="last"){$Mc=$g->result(count_rows($a,$Z,$wd,$Tc));$E=floor(max(0,$Mc-1)/$z);}$lg=$M;if(!$lg){$lg[]="*";if($Be)$lg[]=$Be;}$vb=convert_fields($f,$o,$M);if($vb)$lg[]=substr($vb,2);$I=$l->select($a,$lg,$Z,$Tc,$Qe,$z,$E,true);if(!$I)echo"<p class='error'>".error()."\n";else{if($w=="mssql"&&$E)$I->seek($z*$E);$gc=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($E&&$w=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last"&&+$z&&$Tc&&$wd&&$w=="sql")$Mc=$g->result(" SELECT FOUND_ROWS()");if(!$L)echo"<p class='message'>".'Å½Ã¡dnÃ© Å™Ã¡dky.'."\n";else{$Na=$b->backwardKeys($a,$Mg);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$Tc&&$M?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'ZmÄ›nit'."</a>");$se=array();$Qc=array();reset($M);$Jf=1;foreach($L[0]as$x=>$X){if($x!=$Be){$X=$_GET["columns"][key($M)];$n=$o[$M?($X?$X["col"]:current($M)):$x];$C=($n?$b->fieldName($n,$Jf):($X["fun"]?"*":$x));if($C!=""){$Jf++;$se[$x]=$C;$e=idf_escape($x);$ed=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($x);$Lb="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($ed.($Qe[0]==$e||$Qe[0]==$x||(!$Qe&&$wd&&$Tc[0]==$e)?$Lb:'')).'">';echo
apply_sql_function($X["fun"],$C)."</a>";echo"<span class='column hidden'>","<a href='".h($ed.$Lb)."' title='".'sestupnÄ›'."' class='text'> â†“</a>";if(!$X["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($x)).'\'); return false;" title="'.'Vyhledat'.'" class="text jsonly"> =</a>';echo"</span>";}$Qc[$x]=$X["fun"];next($M);}}$Qd=array();if($_GET["modify"]){foreach($L
as$K){foreach($K
as$x=>$X)$Qd[$x]=max($Qd[$x],min(40,strlen(utf8_decode($X))));}}echo($Na?"<th>".'Vztahy':"")."</thead>\n";if(is_ajax()){if($z%2==1&&$E%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($L,$Jc)as$re=>$K){$yh=unique_array($L[$re],$v);if(!$yh){$yh=array();foreach($L[$re]as$x=>$X){if(!preg_match('~^(COUNT\\((\\*|(DISTINCT )?`(?:[^`]|``)+`)\\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\\(`(?:[^`]|``)+`\\))$~',$x))$yh[$x]=$X;}}$zh="";foreach($yh
as$x=>$X){if(($w=="sql"||$w=="pgsql")&&strlen($X)>64){$x="MD5(".(strpos($x,'(')?$x:idf_escape($x)).")";$X=md5($X);}$zh.="&".($X!==null?urlencode("where[".bracket_escape($x)."]")."=".urlencode($X):"null%5B%5D=".urlencode($x));}echo"<tr".odd().">".(!$Tc&&$M?"":"<td>".checkbox("check[]",substr($zh,1),in_array(substr($zh,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($wd||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$zh)."'>".'upravit'."</a>"));foreach($K
...
as$x=>$X)json_row("sum-$x",format_number($X));json_row("");}elseif($_GET["script"]=="kill")$g->query("KILL ".(+$_POST["kill"]));else{foreach(count_tables($b->databases())as$k=>$X){json_row("tables-$k",$X);json_row("size-$k",db_size($k));}json_row("");}exit;}else{$Sg=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($Sg&&!$m&&!$_POST["search"]){$I=true;$ge="";if($w=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$I=truncate_tables($_POST["tables"]);$ge='Tabulky byly vyprÃ¡zdnÄ›ny.';}elseif($_POST["move"]){$I=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ge='Tabulky byly pÅ™esunuty.';}elseif($_POST["copy"]){$I=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ge='Tabulky byly zkopÃ­rovÃ¡ny.';}elseif($_POST["drop"]){if($_POST["views"])$I=drop_views($_POST["views"]);if($I&&$_POST["tables"])$I=drop_tables($_POST["tables"]);$ge='Tabulky byly odstranÄ›ny.';}elseif($w!="sql"){$I=($w=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$ge='Tabulky byly optimalizovÃ¡ny.';}elseif(!$_POST["tables"])$ge='Å½Ã¡dnÃ© tabulky.';elseif($I=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"])))){while($K=$I->fetch_assoc())$ge.="<b>".h($K["Table"])."</b>: ".h($K["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$ge,$I);}page_header(($_GET["ns"]==""?'DatabÃ¡ze'.": ".h(DB):'SchÃ©ma'.": ".h($_GET["ns"])),$m,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3 id='tables-views'>".'Tabulky a pohledy'."</h3>\n";$Rg=tables_list();if(!$Rg)echo"<p class='message'>".'Å½Ã¡dnÃ© tabulky.'."\n";else{echo"<form action='' method='post'>\n";if(support("table")){echo"<fieldset><legend>".'Vyhledat data v tabulkÃ¡ch'." <span id='selected2'></span></legend><div>","<input type='search' name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Vyhledat'."'>\n","</div></fieldset>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();}echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabulka','<td>'.'ÃšloÅ¾iÅ¡tÄ›','<td>'.'PorovnÃ¡vÃ¡nÃ­','<td>'.'Velikost dat','<td>'.'Velikost indexÅ¯','<td>'.'VolnÃ© mÃ­sto','<td>'.'Auto Increment','<td>'.'Å˜Ã¡dkÅ¯',(support("comment")?'<td>'.'KomentÃ¡Å™':''),"</thead>\n";$S=0;foreach($Rg
as$C=>$U){$Ph=($U!==null&&!preg_match('~table~i',$U));echo'<tr'.odd().'><td>'.checkbox(($Ph?"views[]":"tables[]"),$C,in_array($C,$Sg,true),"","formUncheck('check-all');"),'<th>'.(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($C).'" title="'.'Zobrazit strukturu'.'">'.h($C).'</a>':h($C));if($Ph){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($C).'" title="'.'PozmÄ›nit pohled'.'">'.'Pohled'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($C).'" title="'.'Vypsat data'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'PozmÄ›nit tabulku'),"Index_length"=>array("indexes",'PozmÄ›nit indexy'),"Data_free"=>array("edit",'NovÃ¡ poloÅ¾ka'),"Auto_increment"=>array("auto_increment=1&create",'PozmÄ›nit tabulku'),"Rows"=>array("select",'Vypsat data'),)as$x=>$_){$s=" id='$x-".h($C)."'";echo($_?"<td align='right'>".(support("table")||$x=="Rows"||(support("indexes")&&$x!="Data_length")?"<a href='".h(ME."$_[0]=").urlencode($C)."'$s title='$_[1]'>?</a>":"<span$s>?</span>"):"<td id='$x-".h($C)."'>&nbsp;");}$S++;}echo(support("comment")?"<td id='Comment-".h($C)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d celkem',count($Rg)),"<td>".nbsp($w=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$x)echo"<td align='right' id='sum-$x'>&nbsp;";echo"</table>\n";if(!information_schema(DB)){$Jh="<input type='submit' value='".'VyÄistit'."'".on_help("'VACUUM'")."> ";$Me="<input type='submit' name='optimize' value='".'Optimalizovat'."'".on_help($w=="sql"?"'OPTIMIZE TABLE'":"'VACUUM OPTIMIZE'")."> ";echo"<fieldset><legend>".'OznaÄenÃ©'." <span id='selected'></span></legend><div>".($w=="sqlite"?$Jh:($w=="pgsql"?$Jh.$Me:($w=="sql"?"<input type='submit' value='".'Analyzovat'."'".on_help("'ANALYZE TABLE'")."> ".$Me."<input type='submit' name='check' value='".'Zkontrolovat'."'".on_help("'CHECK TABLE'")."> "."<input type='submit' name='repair' value='".'Opravit'."'".on_help("'REPAIR TABLE'")."> ":"")))."<input type='submit' name='truncate' value='".'VyprÃ¡zdnit'."'".confirm().on_help($w=="sqlite"?"'DELETE'":"'TRUNCATE".($w=="pgsql"?"'":" TABLE'"))."> "."<input type='submit' name='drop' value='".'Odstranit'."'".confirm().on_help("'DROP TABLE'").">\n";$j=(support("scheme")?$b->schemas():$b->databases());if(count($j)!=1&&$w!="sqlite"){$k=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'PÅ™esunout do jinÃ© databÃ¡ze'.": ",($j?html_select("target",$j,$k):'<input name="target" value="'.h($k).'" autocapitalize="off">')," <input type='submit' name='move' value='".'PÅ™esunout'."'>",(support("copy")?" <input type='submit' name='copy' value='".'ZkopÃ­rovat'."'>":""),"\n";}echo"<input type='hidden' name='all' value='' onclick=\"selectCount('selected', formChecked(this, /^(tables|views)\[/));".(support("table")?" selectCount('selected2', formChecked(this, /^tables\[/) || $S);":"")."\">\n";echo"<input type='hidden' name='token' value='$T'>\n","</div></fieldset>\n";}echo"</form>\n","<script type='text/javascript'>tableCheck();</script>\n";}echo'<p class="links"><a href="'.h(ME).'create=">'.'VytvoÅ™it tabulku'."</a>\n",(support("view")?'<a href="'.h(ME).'view=">'.'VytvoÅ™it pohled'."</a>\n":"");if(support("routine")){echo"<h3 id='routines'>".'Procedury a funkce'."</h3>\n";$dg=routines();if($dg){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'NÃ¡zev'.'<td>'.'Typ'.'<td>'.'NÃ¡vratovÃ½ typ'."<td>&nbsp;</thead>\n";odd('');foreach($dg
