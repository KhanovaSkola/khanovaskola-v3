...
upload_error($k){$Ec=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Hochladen von Datei fehlgeschlagen.'.($Ec?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$Ec):""):'Datei existiert nicht.');}function
...
as$D=>$l){if(!isset($l["privileges"][$Be?"update":"insert"])||$b->fieldName($l)=="")unset($m[$D]);}if($_POST&&!$k&&!isset($_GET["select"])){$_=$_POST["referer"];if($_POST["insert"])$_=($Be?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$_))$_=ME."select=".urlencode($a);$t=indexes($a);$xe=unique_array($_GET["where"],$t);$td="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($_,'Datensatz gelöscht.',$i->delete($a,$td,!$xe));else{$Q=array();foreach($m
as$D=>$l){$W=process_input($l);if($W!==false&&$W!==null)$Q[idf_escape($D)]=$W;}if($Be){if(!$Q)redirect($_);queries_redirect($_,'Datensatz geändert.',$i->update($a,$Q,$td,!$xe));if(is_ajax()){page_headers();page_messages($k);exit;}}else{$K=$i->insert($a,$Q);$vc=($K?last_id():0);queries_redirect($_,sprintf('Datensatz%s hinzugefügt.',($vc?" $vc":"")),$K);}}}$M=null;if($_POST["save"])$M=(array)$_POST["fields"];elseif($Z){$O=array();foreach($m
