...
enum_input($se,$c,$l,$X,$rb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$Bc);$L=($rb!==null?"<label><input type='$se'$c value='$rb'".((is_array($X)?in_array($rb,$X):$X===0)?" checked":"")."><i>".'leer'."</i></label>":"");foreach($Bc[1]as$q=>$W){$W=stripcslashes(str_replace("''","'",$W));$Ea=(is_int($X)?$X==$q+1:(is_array($X)?in_array($q+1,$X):$X===$W));$L.=" <label><input type='$se'$c value='".($q+1)."'".($Ea?' checked':'').'>'.h($b->editVal($W,$l)).'</label>';}return$L;}function
...
selectSearchPrint($Y,$e,$t){$Y=(array)$_GET["where"];echo'<fieldset id="fieldset-search"><legend>'.'Suchen'."</legend><div>\n";$qc=array();foreach($Y
as$x=>$W)$qc[$W["col"]]=$x;$q=0;$m=fields($_GET["select"]);foreach($e
as$D=>$cb){$l=$m[$D];if(preg_match("~enum~",$l["type"])||like_bool($l)){$x=$qc[$D];$q--;echo"<div>".h($cb)."<input type='hidden' name='where[$q][col]' value='".h($D)."'>:",(like_bool($l)?" <select name='where[$q][val]'>".optionlist(array(""=>"",'no','yes'),$Y[$x]["val"],true)."</select>":enum_input("checkbox"," name='where[$q][val][]'",$l,(array)$Y[$x]["val"],($l["null"]?0:null))),"</div>\n";unset($e[$D]);}elseif(is_array($Xc=$this->_foreignKeyOptions($_GET["select"],$D))){if($m[$D]["null"])$Xc[0]='('.'leer'.')';$x=$qc[$D];$q--;echo"<div>".h($cb)."<input type='hidden' name='where[$q][col]' value='".h($D)."'><input type='hidden' name='where[$q][op]' value='='>: <select name='where[$q][val]'>".optionlist($Xc,$Y[$x]["val"],true)."</select></div>\n";unset($e[$D]);}}$q=0;foreach($Y
...
as$pc=>$W){if($W[0]<$he)unset($oc[$pc]);}}$nc=&$oc[$b->bruteForceKey()];if(!$nc)$nc=array($he+30*60,0);$nc[1]++;$Kd=serialize($oc);rewind($Qb);fwrite($Qb,$Kd);ftruncate($Qb,strlen($Kd));flock($Qb,LOCK_UN);fclose($Qb);}$ra=$_POST["auth"];if($ra){$oc=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$nc=$oc[$b->bruteForceKey()];$Mc=($nc[1]>30?$nc[0]-time():0);if($Mc>0)auth_error(sprintf('Too many unsuccessful logins, try again in %d minute(s).',ceil($Mc/60)));session_regenerate_id();$i=$ra["driver"];$P=$ra["server"];$U=$ra["username"];$H=(string)$ra["password"];$h=$ra["db"];set_password($i,$P,$U,$H);$_SESSION["db"][$i][$P][$U][$h]=true;if($ra["permanent"]){$x=base64_encode($i)."-".base64_encode($P)."-".base64_encode($U)."-".base64_encode($h);$nd=$b->permanentLogin(true);$fd[$x]="$x:".base64_encode($nd?encrypt_string($H,$nd):"");cookie("adminer_permanent",implode(" ",$fd));}if(count($_POST)==1||DRIVER!=$i||SERVER!=$P||$_GET["username"]!==$U||DB!=$h)redirect(auth_url($i,$P,$U,$h));}elseif($_POST["logout"]){if($Zb&&!verify_token()){page_header('Abmelden','CSRF Token ungÃ¼ltig. Bitte die Formulardaten erneut abschicken.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$x)set_session($x,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'Abmeldung erfolgreich.');}}elseif($fd&&!$_SESSION["pwds"]){session_regenerate_id();$nd=$b->permanentLogin();foreach($fd
...
auth_error($k){global$b,$Zb;$Nd=session_name();if(!$_COOKIE[$Nd]&&$_GET[$Nd]&&ini_bool("session.use_only_cookies"))$k='Sitzungen mÃ¼ssen aktiviert sein.';elseif(isset($_GET["username"])){if(($_COOKIE[$Nd]||$_GET[$Nd])&&!$Zb)$k='Sitzungsdauer abgelaufen, bitte erneut anmelden.';else{add_invalid_login();$H=get_password();if($H!==null){if($H===false)$k.='<br>'.sprintf('Master password expired. <a href="http://www.adminer.org/en/extension/" target="_blank">Implement</a> %s method to make it permanent.','<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}$dd=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$dd["lifetime"]);page_header('Login',$k,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Keine Erweiterungen installiert',sprintf('Keine der unterstÃ¼tzten PHP-Erweiterungen (%s) ist vorhanden.',implode(", ",$jd)),false);page_footer("auth");exit;}$f=connect();}$i=new
Min_Driver($f);if(!is_object($f)||!$b->login($_GET["username"],get_password()))auth_error((is_string($f)?$f:'UngÃ¼ltige Anmelde-Informationen.'));if($ra&&$_POST["token"])$_POST["token"]=$oe;$k='';if($_POST){if(!verify_token()){$kc="max_input_vars";$Fc=ini_get($kc);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$x){$W=ini_get($x);if($W&&(!$Fc||$W<$Fc)){$kc=$x;$Fc=$W;}}}$k=(!$_POST["token"]&&$Fc?sprintf('Die maximal erlaubte Anzahl der Felder ist Ã¼berschritten. Bitte %s erhÃ¶hen.',"'$kc'"):'CSRF Token ungÃ¼ltig. Bitte die Formulardaten erneut abschicken.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$k=sprintf('POST data zu gross. Reduzieren Sie die GrÃ¶sse oder vergrÃ¶ssern Sie den Wert %s in der Konfiguration.',"'post_max_size'");if(isset($_GET["sql"]))$k.=' '.'You can upload a big SQL file via FTP and import it from server.';}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
...
as$D=>$l){if(!isset($l["privileges"][$Be?"update":"insert"])||$b->fieldName($l)=="")unset($m[$D]);}if($_POST&&!$k&&!isset($_GET["select"])){$_=$_POST["referer"];if($_POST["insert"])$_=($Be?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$_))$_=ME."select=".urlencode($a);$t=indexes($a);$xe=unique_array($_GET["where"],$t);$td="\nWHERE $Y";if(isset($_POST["delete"]))queries_redirect($_,'Datensatz gelÃ¶scht.',$i->delete($a,$td,!$xe));else{$Q=array();foreach($m
as$D=>$l){$W=process_input($l);if($W!==false&&$W!==null)$Q[idf_escape($D)]=$W;}if($Be){if(!$Q)redirect($_);queries_redirect($_,'Datensatz geÃ¤ndert.',$i->update($a,$Q,$td,!$xe));if(is_ajax()){page_headers();page_messages($k);exit;}}else{$K=$i->insert($a,$Q);$vc=($K?last_id():0);queries_redirect($_,sprintf('Datensatz%s hinzugefÃ¼gt.',($vc?" $vc":"")),$K);}}}$M=null;if($_POST["save"])$M=(array)$_POST["fields"];elseif($Y){$O=array();foreach($m
...
as$x=>$W){if(($v=="sql"||$v=="pgsql")&&strlen($W)>64){$x="MD5(".(strpos($x,'(')?$x:idf_escape($x)).")";$W=md5($W);}$ye.="&".($W!==null?urlencode("where[".bracket_escape($x)."]")."=".urlencode($W):"null%5B%5D=".urlencode($x));}echo"<tr".odd().">".(!$p&&$O?"":"<td>".checkbox("check[]",substr($ye,1),in_array(substr($ye,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($u||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$ye)."'>".'Ã¤ndern'."</a>"));foreach($M
