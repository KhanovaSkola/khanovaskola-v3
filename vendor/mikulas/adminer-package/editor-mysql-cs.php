...
as$D=>$cb){$l=$m[$D];if(preg_match("~enum~",$l["type"])||like_bool($l)){$x=$qc[$D];$q--;echo"<div>".h($cb)."<input type='hidden' name='where[$q][col]' value='".h($D)."'>:",(like_bool($l)?" <select name='where[$q][val]'>".optionlist(array(""=>"",'ne','ano'),$Y[$x]["val"],true)."</select>":enum_input("checkbox"," name='where[$q][val][]'",$l,(array)$Y[$x]["val"],($l["null"]?0:null))),"</div>\n";unset($e[$D]);}elseif(is_array($Xc=$this->_foreignKeyOptions($_GET["select"],$D))){if($m[$D]["null"])$Xc[0]='('.'prÃ¡zdnÃ©'.')';$x=$qc[$D];$q--;echo"<div>".h($cb)."<input type='hidden' name='where[$q][col]' value='".h($D)."'><input type='hidden' name='where[$q][op]' value='='>: <select name='where[$q][val]'>".optionlist($Xc,$Y[$x]["val"],true)."</select></div>\n";unset($e[$D]);}}$q=0;foreach($Y
...
as$x=>$s){$E=array();foreach($s["columns"]as$W)$E[]=$e[$W];if(count(array_filter($E,'strlen'))>1&&$x!="PRIMARY")$Zc[$x]=implode(", ",$E);}if($Zc){echo'<fieldset><legend>'.'SeÅ™adit'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Zc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
as$pc=>$W){if($W[0]<$he)unset($oc[$pc]);}}$nc=&$oc[$b->bruteForceKey()];if(!$nc)$nc=array($he+30*60,0);$nc[1]++;$Kd=serialize($oc);rewind($Qb);fwrite($Qb,$Kd);ftruncate($Qb,strlen($Kd));flock($Qb,LOCK_UN);fclose($Qb);}$ra=$_POST["auth"];if($ra){$oc=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$nc=$oc[$b->bruteForceKey()];$Mc=($nc[1]>30?$nc[0]-time():0);if($Mc>0)auth_error(lang(array('PÅ™Ã­liÅ¡ mnoho pokusÅ¯ o pÅ™ihlÃ¡Å¡enÃ­, zkuste to znovu za %d minutu.','PÅ™Ã­liÅ¡ mnoho pokusÅ¯ o pÅ™ihlÃ¡Å¡enÃ­, zkuste to znovu za %d minuty.','PÅ™Ã­liÅ¡ mnoho pokusÅ¯ o pÅ™ihlÃ¡Å¡enÃ­, zkuste to znovu za %d minut.'),ceil($Mc/60)));session_regenerate_id();$i=$ra["driver"];$P=$ra["server"];$U=$ra["username"];$H=(string)$ra["password"];$h=$ra["db"];set_password($i,$P,$U,$H);$_SESSION["db"][$i][$P][$U][$h]=true;if($ra["permanent"]){$x=base64_encode($i)."-".base64_encode($P)."-".base64_encode($U)."-".base64_encode($h);$nd=$b->permanentLogin(true);$fd[$x]="$x:".base64_encode($nd?encrypt_string($H,$nd):"");cookie("adminer_permanent",implode(" ",$fd));}if(count($_POST)==1||DRIVER!=$i||SERVER!=$P||$_GET["username"]!==$U||DB!=$h)redirect(auth_url($i,$P,$U,$h));}elseif($_POST["logout"]){if($Zb&&!verify_token()){page_header('OdhlÃ¡sit','NeplatnÃ½ token CSRF. OdeÅ¡lete formulÃ¡Å™ znovu.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$x)set_session($x,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'OdhlÃ¡Å¡enÃ­ probÄ›hlo v poÅ™Ã¡dku.');}}elseif($fd&&!$_SESSION["pwds"]){session_regenerate_id();$nd=$b->permanentLogin();foreach($fd
...
auth_error($k){global$b,$Zb;$Nd=session_name();if(!$_COOKIE[$Nd]&&$_GET[$Nd]&&ini_bool("session.use_only_cookies"))$k='Session promÄ›nnÃ© musÃ­ bÃ½t povolenÃ©.';elseif(isset($_GET["username"])){if(($_COOKIE[$Nd]||$_GET[$Nd])&&!$Zb)$k='Session vyprÅ¡ela, pÅ™ihlaÅ¡te se prosÃ­m znovu.';else{add_invalid_login();$H=get_password();if($H!==null){if($H===false)$k.='<br>'.sprintf('Platnost hlavnÃ­ho hesla vyprÅ¡ela. <a href="http://www.adminer.org/cs/extension/" target="_blank">Implementujte</a> metodu %s, aby platilo stÃ¡le.','<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}$dd=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$dd["lifetime"]);page_header('PÅ™ihlÃ¡sit se',$k,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Å½Ã¡dnÃ© rozÅ¡Ã­Å™enÃ­',sprintf('NenÃ­ dostupnÃ© Å¾Ã¡dnÃ© z podporovanÃ½ch PHP rozÅ¡Ã­Å™enÃ­ (%s).',implode(", ",$jd)),false);page_footer("auth");exit;}$f=connect();}$i=new
Min_Driver($f);if(!is_object($f)||!$b->login($_GET["username"],get_password()))auth_error((is_string($f)?$f:'NeplatnÃ© pÅ™ihlaÅ¡ovacÃ­ Ãºdaje.'));if($ra&&$_POST["token"])$_POST["token"]=$oe;$k='';if($_POST){if(!verify_token()){$kc="max_input_vars";$Fc=ini_get($kc);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$x){$W=ini_get($x);if($W&&(!$Fc||$W<$Fc)){$kc=$x;$Fc=$W;}}}$k=(!$_POST["token"]&&$Fc?sprintf('Byl pÅ™ekroÄen maximÃ¡lnÃ­ povolenÃ½ poÄet polÃ­. ZvyÅ¡te prosÃ­m %s.',"'$kc'"):'NeplatnÃ½ token CSRF. OdeÅ¡lete formulÃ¡Å™ znovu.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$k=sprintf('PÅ™Ã­liÅ¡ velkÃ¡ POST data. ZmenÅ¡ete data nebo zvyÅ¡te hodnotu konfiguraÄnÃ­ direktivy %s.',"'post_max_size'");if(isset($_GET["sql"]))$k.=' '.'VelkÃ½ SQL soubor mÅ¯Å¾ete nahrÃ¡t pomocÃ­ FTP a importovat ho ze serveru.';}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
