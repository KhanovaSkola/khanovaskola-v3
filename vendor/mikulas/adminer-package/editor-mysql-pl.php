...
enum_input($se,$c,$l,$X,$rb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$Bc);$L=($rb!==null?"<label><input type='$se'$c value='$rb'".((is_array($X)?in_array($rb,$X):$X===0)?" checked":"")."><i>".'puste'."</i></label>":"");foreach($Bc[1]as$q=>$W){$W=stripcslashes(str_replace("''","'",$W));$Ea=(is_int($X)?$X==$q+1:(is_array($X)?in_array($q+1,$X):$X===$W));$L.=" <label><input type='$se'$c value='".($q+1)."'".($Ea?' checked':'').'>'.h($b->editVal($W,$l)).'</label>';}return$L;}function
...
as$D=>$cb){$l=$m[$D];if(preg_match("~enum~",$l["type"])||like_bool($l)){$x=$qc[$D];$q--;echo"<div>".h($cb)."<input type='hidden' name='where[$q][col]' value='".h($D)."'>:",(like_bool($l)?" <select name='where[$q][val]'>".optionlist(array(""=>"",'nie','tak'),$Y[$x]["val"],true)."</select>":enum_input("checkbox"," name='where[$q][val][]'",$l,(array)$Y[$x]["val"],($l["null"]?0:null))),"</div>\n";unset($e[$D]);}elseif(is_array($Xc=$this->_foreignKeyOptions($_GET["select"],$D))){if($m[$D]["null"])$Xc[0]='('.'puste'.')';$x=$qc[$D];$q--;echo"<div>".h($cb)."<input type='hidden' name='where[$q][col]' value='".h($D)."'><input type='hidden' name='where[$q][op]' value='='>: <select name='where[$q][val]'>".optionlist($Xc,$Y[$x]["val"],true)."</select></div>\n";unset($e[$D]);}}$q=0;foreach($Y
...
as$pc=>$W){if($W[0]<$he)unset($oc[$pc]);}}$nc=&$oc[$b->bruteForceKey()];if(!$nc)$nc=array($he+30*60,0);$nc[1]++;$Kd=serialize($oc);rewind($Qb);fwrite($Qb,$Kd);ftruncate($Qb,strlen($Kd));flock($Qb,LOCK_UN);fclose($Qb);}$ra=$_POST["auth"];if($ra){$oc=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$nc=$oc[$b->bruteForceKey()];$Mc=($nc[1]>30?$nc[0]-time():0);if($Mc>0)auth_error(lang(array('Za duÅ¼o nieudanych prÃ³b logowania, sprÃ³buj ponownie za %d minutÄ™.','Za duÅ¼o nieudanych prÃ³b logowania, sprÃ³buj ponownie za %d minuty.','Za duÅ¼o nieudanych prÃ³b logowania, sprÃ³buj ponownie za %d minut.'),ceil($Mc/60)));session_regenerate_id();$i=$ra["driver"];$P=$ra["server"];$U=$ra["username"];$H=(string)$ra["password"];$h=$ra["db"];set_password($i,$P,$U,$H);$_SESSION["db"][$i][$P][$U][$h]=true;if($ra["permanent"]){$x=base64_encode($i)."-".base64_encode($P)."-".base64_encode($U)."-".base64_encode($h);$nd=$b->permanentLogin(true);$fd[$x]="$x:".base64_encode($nd?encrypt_string($H,$nd):"");cookie("adminer_permanent",implode(" ",$fd));}if(count($_POST)==1||DRIVER!=$i||SERVER!=$P||$_GET["username"]!==$U||DB!=$h)redirect(auth_url($i,$P,$U,$h));}elseif($_POST["logout"]){if($Zb&&!verify_token()){page_header('Wyloguj','NieprawidÅ‚owy token CSRF. SprÃ³buj wysÅ‚aÄ‡ formularz ponownie.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$x)set_session($x,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'Wylogowano pomyÅ›lnie.');}}elseif($fd&&!$_SESSION["pwds"]){session_regenerate_id();$nd=$b->permanentLogin();foreach($fd
...
auth_error($k){global$b,$Zb;$Nd=session_name();if(!$_COOKIE[$Nd]&&$_GET[$Nd]&&ini_bool("session.use_only_cookies"))$k='Wymagana jest obsÅ‚uga sesji w PHP.';elseif(isset($_GET["username"])){if(($_COOKIE[$Nd]||$_GET[$Nd])&&!$Zb)$k='Sesja wygasÅ‚a, zaloguj siÄ™ ponownie.';else{add_invalid_login();$H=get_password();if($H!==null){if($H===false)$k.='<br>'.sprintf('WaÅ¼noÅ›Ä‡ hasÅ‚a gÅ‚Ã³wnego wygasÅ‚a. <a href="http://www.adminer.org/pl/extension/" target="_blank">Zaimplementuj</a> wÅ‚asnÄ… metodÄ™ %s, aby ustawiÄ‡ je na staÅ‚e.','<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}$dd=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$dd["lifetime"]);page_header('Zaloguj siÄ™',$k,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Brak rozszerzenia',sprintf('Å»adne z rozszerzeÅ„ PHP umoÅ¼liwiajÄ…cych poÅ‚Ä…czenie siÄ™ z bazÄ… danych (%s) nie jest dostÄ™pne.',implode(", ",$jd)),false);page_footer("auth");exit;}$f=connect();}$i=new
